...
$$
\frac{\mathbf{\overline{X}}}{1 + \frac{z_\beta \theta}{ \sqrt{n}}} \leq \theta \leq \frac{\mathbf{\overline{X}}}{1 - \frac{z_\beta \theta}{ \sqrt{n}}}
...
for (n in sizes){
  for (m in repetitions){
    for (a in alphas) {
      ...
      z_stat_intervals[counter, ] <- list(n, m, a, z_s$ciq, z_s$cix, z_s$cim) 
      chi_sq_intervals[counter, ] <- list(n, m, a, c_s$ciq, c_s$cix, c_s$cim)
      z_indp_intervals[counter, ] <- list(n, m, a, i_s$ciq, i_s$cix, i_s$cim) 
      t_stud_intervals[counter, ] <- list(n, m, a, t_s$ciq, t_s$cix, t_s$cim) 
      counter <- counter + 1
    }
  }
...
```{r}
all_intervals <- list(
  "Z-stat" = z_stat_intervals,
  "Chi-squared" = chi_sq_intervals,
  "Z-stat-indp" = z_indp_intervals,
  "T-stud" = t_stud_intervals
)

for (name in names(all_intervals)) {
  interval <- all_intervals[[name]]
  cat("Interval type:", name, "CI quality on average:", mean(interval[, 4]), "\n")
}
```
